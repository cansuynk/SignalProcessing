...
#Carrier frequency 10Hz higher
fc2 = 8510
phase = 0 #We have phase
...
plots[0].set_title("The Signal After Filtering (carrier frequency 10 Hz higher)")
plots[0].plot(v_t, color='C9')
plots[0].set_xlim(beginning_pt, end_pt)

plots[1].plot(abs(v_t), color='C9')
plots[1].set_xlim(beginning_pt, end_pt)

plt.show()
s.close()
#v_t = np.asarray(v_t, dtype=np.int16)
#scipy.io.wavfile.write('ses9.wav', sampling_rate, v_t)